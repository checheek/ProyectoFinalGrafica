...
/* ----------------   Práctica 5 --------------------------*/
/*-----------------    2023-1   ---------------------------*/
/*------- Rodríguez López Araiza José Eduardo -------------*/
/*---------   No. de cuenta:  315226742     ---------------*/
#include <glew.h>
#include <glfw3.h>
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		// initialize Matrix, Use this matrix for individual models
		...
		glm::mat4 temp01 = modelOp = glm::translate(modelOp, glm::vec3(1.0f, 0.0f, 0.0f));
		...
		glm::mat4 temp02 = modelOp = glm::translate(modelOp, glm::vec3(1.25f, 0.0f, 0.0f));
		...
		modelOp = glm::rotate(modelOp, glm::radians(dedo1pt1), glm::vec3(0.0f, 1.0f, 0.0f));
		temp01 = modelOp = glm::translate(modelOp, glm::vec3(0.25f, 0.0f, 0.0f));
		...
		modelOp = glm::translate(temp01, glm::vec3(0.25f, 0.0f, 0.0f));
		modelOp = glm::rotate(modelOp, glm::radians(dedo1pt2), glm::vec3(0.0f, 1.0f, 0.0f));
		temp01 = modelOp = glm::translate(modelOp, glm::vec3(0.25f, 0.0f, 0.0f));
		modelOp = glm::scale(modelOp, glm::vec3(0.5f, 0.3f, 0.25f));
		myShader.setMat4("model", modelOp);
		...
		modelOp = glm::translate(temp02, glm::vec3(1.25f, 0.35f, 0.375f));
		modelOp = glm::rotate(modelOp, glm::radians(-dedo1pt1), glm::vec3(0.0f, 0.0f, 1.0f));
		temp01 = modelOp = glm::translate(modelOp, glm::vec3(0.25f, 0.0f, 0.0f));
		modelOp = glm::scale(modelOp, glm::vec3(0.5f, 0.3f, 0.25f));
		myShader.setMat4("model", modelOp);
		myShader.setVec3("aColor", glm::vec3(0.0f, 1.0f, 0.0f));
		...
		modelOp = glm::translate(temp01, glm::vec3(0.25f, 0.0f, 0.0f));
		modelOp = glm::rotate(modelOp, glm::radians(-dedo1pt2), glm::vec3(0.0f, 0.0f, 1.0f));
		temp01 = modelOp = glm::translate(modelOp, glm::vec3(0.375f, 0.0f, 0.0f));
		modelOp = glm::scale(modelOp, glm::vec3(0.75f, 0.3f, 0.25f));
		myShader.setMat4("model", modelOp);
		myShader.setVec3("aColor", glm::vec3(0.0f, 0.0f, 1.0f));
		glDrawArrays(GL_TRIANGLES, 0, 36);	//Dibujo de D

		modelOp = glm::translate(temp02, glm::vec3(1.25f, 0.35f, 0.0f));
		modelOp = glm::rotate(modelOp, glm::radians(-dedo1pt1), glm::vec3(0.0f, 0.0f, 1.0f));
		temp01 = modelOp = glm::translate(modelOp, glm::vec3(0.375f, 0.0f, 0.0f));
		modelOp = glm::scale(modelOp, glm::vec3(0.75f, 0.3f, 0.25f));
		myShader.setMat4("model", modelOp);
		...
		modelOp = glm::translate(temp01, glm::vec3(0.375f, 0.0f, 0.0f));
		modelOp = glm::rotate(modelOp, glm::radians(-dedo1pt2), glm::vec3(0.0f, 0.0f, 1.0f));
		temp01 = modelOp = glm::translate(modelOp, glm::vec3(0.5f, 0.0f, 0.0f));
...
void my_input(GLFWwindow *window)
{
	...
	if (glfwGetKey(window, GLFW_KEY_R) == GLFW_PRESS && hombro < 90.0f)
		hombro += 0.5f;
	if (glfwGetKey(window, GLFW_KEY_F) == GLFW_PRESS && hombro > -120.0f)
		hombro -= 0.5f;
	if (glfwGetKey(window, GLFW_KEY_T) == GLFW_PRESS && codo < 15.0f)
		codo += 0.5f;
	if (glfwGetKey(window, GLFW_KEY_G) == GLFW_PRESS && codo > -100.0f)
		codo -= 0.5f;
	if (glfwGetKey(window, GLFW_KEY_Y) == GLFW_PRESS && dedo1pt1 < 15.0f)
		dedo1pt1 += 0.5f;
	if (glfwGetKey(window, GLFW_KEY_H) == GLFW_PRESS && dedo1pt1 > -100.0f)
		dedo1pt1 -= 0.5f;

	if (glfwGetKey(window, GLFW_KEY_U) == GLFW_PRESS && dedo1pt2 < 15.0f)
		dedo1pt2 += 0.5f;
	if (glfwGetKey(window, GLFW_KEY_J) == GLFW_PRESS && dedo1pt2 > -100.0f)
		dedo1pt2 -= 0.5f;
